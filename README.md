...
- [x] 텍스트 추가 — 캔버스에서 마우스로 드래그해 위치를 정한 뒤 적용, 시스템 폰트 선택 + 커스텀 폰트 파일 추가, 전용 초기화 버튼
- [x] 보정 (밝기/대비/채도, 필터 프리셋) — 초기화 시 이 도구에 들어왔을 때 상태로 복귀
- [x] 배경 제거 (rembg 자동 제거, 단색 배경 채우기, 마우스로 자유선택한 영역만 지우는 "영역 선택 지우기")
- [x] 지우개 (브러시 드래그로 알파 채널 지우기, 고해상도 사진도 축소 프록시로 부드럽게 처리)
